...
class MetaOptimizerEngine:
    ...
    def generate_strategy_report(self, mode="Clash Squad", strategy_type="Aggressive Attack"):
        ...
        print(f"   Active Meta Version: {self.latest_patch.upper()}                     ")
        print(f"==================================================\n")
        ...
        if k_data:
            print(f"  └ Source: {k_data['patch']} | Interval: 1.0s EP recovery | Cap: 250 EP")
        
        print(f"• Defensive Counter Note: Chrono (Time Turner)")
        if chrono_data:
            print(f"  └ Source: {chrono_data['patch']} | Shield: 800 HP (Two-Way Protection, No inside-out firing)")
